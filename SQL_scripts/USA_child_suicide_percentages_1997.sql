...
FROM child_suicide;
-- suicide_plan = 16135, suicide_attemps = 14764
...
SELECT ((2544)::float / (COUNT(suicide_plan))::float) AS percentage_planned
FROM child_suicide;
-- 15.77%
...
SELECT ((1332)::float / (COUNT(suicide_attempts))::float) AS percentage_attempted
FROM child_suicide;
-- 9.02%
...
SELECT ((5849)::float / (COUNT(fights))::float) AS percentage_fighting
FROM master_df;
-- 36.43%