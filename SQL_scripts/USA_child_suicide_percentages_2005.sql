...
FROM child_suicide;
-- suicide_plan = 13791, suicide_attemps = 12427
...
SELECT ((1835)::float / (COUNT(suicide_plan))::float) AS percentage_planned
FROM child_suicide;
-- 13.31%
...
SELECT ((1121)::float / (COUNT(suicide_attempts))::float) AS percentage_attempted
FROM child_suicide;
-- 9.02%
...
SELECT ((4889)::float / (COUNT(fights))::float) AS percentage_fighting
FROM master_df;
-- 35.67%
...
SELECT ((4156)::float / (COUNT(sad))::float) AS percentage_sad
FROM master_df;
-- 30.16%
